...
- [ ] Tab icons, text colour, background, and badge match legacy
      footer parity for the same input JSON
- [ ] Selected-tab indicator is visible (colour, weight, or fill swap)
- [ ] SF-symbol image URLs render as symbols, not AsyncImage fetches
- [ ] Failed AsyncImage fetch shows a placeholder icon, not an
      invisible tappable area
- [ ] Tests cover selected / unselected / badge / symbol / network-icon

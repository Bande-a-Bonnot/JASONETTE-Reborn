...
export const PRODUCTION_BODY_PIPELINE_CASES = [
  { title: "production body pipeline: body root propagates HTML text mode", path: "body-root" },
  { title: "production body pipeline: header propagates HTML text mode", path: "header" },
  { title: "production body pipeline: footer propagates HTML text mode", path: "footer" },
  { title: "production body pipeline: section propagates HTML text mode", path: "section" },
  { title: "production body pipeline: layout propagates HTML text mode", path: "layout" },
  { title: "production body pipeline: layer propagates HTML text mode", path: "layer" },
  { title: "production body pipeline: background propagates HTML text mode", path: "background" },
  { title: "production body pipeline: nested arrays propagate HTML text mode", path: "nested-array" },
  { title: "production body pipeline: directives propagate HTML text mode", path: "directive" },
  { title: "production body pipeline: embedded action propagates HTML text mode", path: "action" },
  { title: "production body pipeline: embedded action options propagate HTML text mode", path: "action-options" },
  { title: "production body pipeline: embedded action payload propagates HTML text mode", path: "action-payload" },
];
...
export const ORDINARY_DESCRIPTOR_CASES = [
  { title: "ordinary transform output field has the exact own data descriptor", kind: "ordinary" },
  { title: "resolved transform output field has the exact own data descriptor", kind: "resolved" },
];

export const NESTED_OFF_MODE_ORDER_CASES = [
  { title: "nested omitted off mode preserves per-entry key-then-value order exactly once in every frame", mode: "omitted" },
  { title: "nested explicit false off mode preserves per-entry key-then-value order exactly once in every frame", mode: "explicit-false" },
];
...
export const COMPONENT_SOURCE_CASES = [
  ...
  { title: "component source: own non-string text yields to an own valid URL without coercion", kind: "non-string-text-url", source: "src", expected: "https://example.com/non-string-text.html" },
...
export const NAMED_ACTION_CASES = [
  ...["toString", "constructor", "__proto__", "prototype"].flatMap((name) => [
...
export const NAMED_ACTION_ARRAY_CASES = [
  { title: "own named action arrays execute every authored entry through trigger", dispatch: "trigger" },
  { title: "own named action arrays execute every authored entry through lambda", dispatch: "lambda" },
];
...
add(ORDINARY_DESCRIPTOR_CASES, ({ kind }) => c(
  kind === "ordinary"
    ? 'input owns ordinary field label="{{value}}" with context value "done"'
    : 'input owns resolved key "{{field}}"="{{value}}" with context field="resolvedLabel" and value="done"',
  "transform through the public template API",
  `output has Object.prototype and own descriptor ${q(kind === "ordinary" ? "label" : "resolvedLabel")}={value:"done", enumerable:true, writable:true, configurable:true}`,
));
add(NESTED_OFF_MODE_ORDER_CASES, ({ mode }) => c(
  `resolved outer child and tail entries plus two resolved inner entries with observable key/value getters, ${mode === "omitted" ? "options omitted" : "preserveHtmlText=false"}`,
  "transform while logging every nested getter access",
  'log equals ["outerKey","innerAKey","innerAValue","innerBKey","innerBValue","tailKey","tailValue"] and every getter count is one',
));
...
add(NAMED_ACTION_ARRAY_CASES, ({ dispatch }) => c(
  `state.actions owns arrayAction with two $set entries producing distinct namedArrayFirstEffect and namedArraySecondEffect values; invocation uses ${dispatch === "trigger" ? '{trigger:"arrayAction"}' : '{$lambda options {name:"arrayAction"}}'}`,
  "dispatch the own named action array",
  'namedArrayFirstEffect equals "first-authored-entry" and namedArraySecondEffect equals "second-authored-entry"',
));
...
const componentSetups = {
  ...
  "non-string-text-url": 'HTML component owns an accessor text returning an object with observable toString and owns URL "https://example.com/non-string-text.html"',
  coercion: "HTML component owns numeric text 17 and an object URL with an observable toString spy",
  "inherited-only": 'HTML component has observable inherited text and URL getters and owns neither source',
  "inherited-text-own-url": 'HTML component has an observable inherited text getter and owns URL "https://example.com/own.html"',
  "inherited-url-own-text": 'HTML component has an observable inherited URL getter and owns text "<p>own</p>"',
  missing: "HTML component owns neither text nor URL",
  invalid: 'HTML component owns text "<p><b>unterminated & raw"',
};
const sourceExpected = (source, expected, kind = "") => {
  const inherited = kind.startsWith("inherited-") ? "; all inherited getter read counts are zero" : "";
  const nonStringFallback = kind === "non-string-text-url" ? "; own text getter read count is one and text toString call count is zero" : "";
  return source === "none"
    ? `no iframe and no observer trace${kind === "coercion" ? "; toString call counts are zero" : ""}${inherited}`
    : `one iframe has sandbox="allow-scripts", only ${source}="${expected}", and events [CREATE(component), SANDBOX("allow-scripts"), SOURCE("${source}"), APPEND, RETURN]${inherited}${nonStringFallback}`;
};
...
add(CSS_CASES, ({ kind, css, expected }) => c(
  `HTML component text is "<p>x</p>" and CSS is ${kind === "absent" ? "absent" : kind === "inherited" ? "an observable inherited getter returning p{color:inherited}" : kind === "coercion" ? "an object with an observable toString" : q(css)}`,
  "render the component through the public component API",
  `iframe srcdoc and srcdoc attribute equal "${expected}"${kind === "coercion" ? " and toString call count is zero" : ""}${kind === "inherited" ? " and inherited CSS getter read count is zero" : ""}${kind === "mixed" || kind === "repeated" ? " with escape code point U+005C" : ""}`, 
));
add(COMPONENT_TYPE_CASES, ({ kind, outcome }) => c(
  `${kind === "missing" ? "component has no type and text \"fallback label\"" : kind === "inherited-html" ? "component has an observable inherited type getter returning \"html\" and owns text \"fallback label\"" : kind === "non-string" ? "component owns numeric type 9 and text \"fallback label\"" : `component owns type ${q(kind)} and text "visible value"`}`,
  "render the component through the public component API",
  outcome === "unknown"
    ? `no throw, iframe, or trace; fallback is a visible DIV with non-empty text, no jasonette-label class, and data-jasonette-type is ${q(kind)}`
    : `no throw, iframe, or trace; result is a P with jasonette-label class, data-jasonette-type="label", and visible text ${kind === "label" ? '"visible value"' : '"fallback label"'}${kind === "inherited-html" ? "; inherited type getter read count is zero" : ""}`, 
));
add(INHERITED_COMPONENT_CALLABLE_CASES, ({ name }) => c(
  ...
  "render the component through the public component API",
  `call count is zero, no throw, iframe, or trace; fallback is a visible DIV with non-empty text, no jasonette-label class, and data-jasonette-type ${q(name)}`,
));
const transformRenderSetups = {
  "all-inherited": 'HTML object owns only type="html" and has observable inherited text, URL, and CSS getters',
  "inherited-text": 'HTML object owns type="html" and URL="https://example.com/transformed-own.html" but has an observable inherited text getter',
  "inherited-url": 'HTML object owns type="html" and text="<p>transformed own</p>" but has an observable inherited URL getter',
  "inherited-css": 'HTML object owns type="html" and text="<p>transformed cssless</p>" but has an observable inherited CSS getter',
};
add(TRANSFORM_RENDER_INHERITANCE_CASES, ({ kind, source, expected }) => c(
  ...
  `${sourceExpected(source, expected)}; all inherited getter read counts are zero`,
));
...
const backgroundSetups = {
  ...
  "inherited-only source fields": 'own type "html" with observable inherited text, URL, and CSS getters only', "inherited text with own URL": 'own type "html" and URL "https://example.com/own-background.html" with an observable inherited text getter',
  "inherited URL with own text": 'own type "html" and text "<p>own background</p>" with an observable inherited URL getter', "inherited CSS with own text": 'own type "html" and text "<p>own cssless</p>" with an observable inherited CSS getter',
  "missing type": 'own text "<p>bad</p>" with no type', "inherited HTML type": 'observable inherited type getter returning "html" and own text "<p>bad</p>"',
...
add(BACKGROUND_MATRIX_CASES, ({ path, id, source, expected }) => c(
  ...
  source === "none" ? `no background iframe and no observer trace${id.includes("coercion") ? "; all toString call counts are zero" : ""}${id.includes("inherited") ? "; all inherited getter read counts are zero" : ""}` : backgroundOutcome(source, expected, `${id.includes("CSS escape") ? ", and the first escape is code point U+005C" : ""}${id.includes("coercion") ? "; all toString call counts are zero" : ""}${id === "non-string own text with valid URL fallback" ? "; text toString call count is zero" : ""}${id.includes("inherited") ? "; all inherited getter read counts are zero" : ""}`), 
));

const productionPipelineSetups = {
  "body-root": 'the body root owns style.background HTML with data-red-location="body-root", raw {{$jason.secret}} text, dynamic CSS height, and a section containing only label "body-root-{{$jason.label}}"',
  header: 'body.header owns a dynamic title and an opaque HTML probe, while body.sections[0].items owns the location-marked rendered components',
  footer: 'body.footer.tabs.items owns location-marked inline HTML, URL HTML, and label components while body.sections is empty',
  section: 'body.sections[0].items directly owns location-marked inline HTML, URL HTML, and label components',
  layout: 'body.sections[0].items[0] is a vertical layout whose components own location-marked inline HTML, URL HTML, and label components',
  layer: 'body.layers[0] is a vertical layer whose components own location-marked inline HTML, URL HTML, and label components, separate from a foreground section',
  background: 'the body root directly owns canonical background HTML with data-red-location="background", raw {{$jason.secret}} text, and dynamic CSS height, plus a foreground section',
  "nested-array": 'body.nested has an opaque HTML shape in two array levels, while body.sections[0].items owns the location-marked rendered components',
  directive: 'body.sections[0].items contains an enabled #if whose result is location-marked inline HTML, URL HTML, and label components',
  action: 'a label control template owns action {type:"{{$jason.actionType}}", options:{actionObjectReached:"{{$jason.label}}"}} with actionType="$set"',
  "action-options": 'a label control template owns a literal $set action whose options.actionOptionsProbe is {type:"html", text:"{{$jason.secret}}", style:{height:"{{$jason.height}}"}}',
  "action-payload": 'a label control template owns a literal $set action whose options.actionPayloadProbe.payload is {type:"html", text:"{{$jason.secret}}", style:{height:"{{$jason.height}}"}}',
};
const productionPipelineExpected = {
  "body-root": 'on initial render and $render, the sole matching jasonette-background-web iframe is a direct renderer-root child, has only srcdoc with data-red-location="body-root", literal {{$jason.secret}}, and CSS heights 31 then 62; no jasonette-html wrapper exists; exact foreground labels are body-root-first then body-root-second',
  background: 'on initial render and $render, the sole matching jasonette-background-web iframe is a direct renderer-root child, has only srcdoc with data-red-location="background", literal {{$jason.secret}}, and CSS heights 31 then 62; no jasonette-html wrapper exists; exact foreground labels are background-first then background-second',
  action: 'clicking the exact rendered controls run-action-first and run-action-second after their respective renders reaches the transformed action object and sets local.actionObjectReached to "first" then "second"',
  "action-options": 'rendered control action data retains literal text {{$jason.secret}} before activation; clicking run-action-options-first and run-action-options-second stores generically transformed text undefined with type="html" and style heights 31 then 62',
  "action-payload": 'rendered control action data retains literal text {{$jason.secret}} before activation; clicking run-action-payload-first and run-action-payload-second stores generically transformed text undefined with type="html" and style heights 31 then 62',
};
const productionPipelineExtra = {
  header: '; transformed header probe retains literal {{$jason.secret}}; header title changes from "header-first" to "header-second"',
  "nested-array": '; after normal array flattening, transformed body.nested[0].text retains literal {{$jason.secret}}',
};
add(PRODUCTION_BODY_PIPELINE_CASES, ({ path }) => c(
  `${productionPipelineSetups[path]}; initial data are secret="FIRST-LEAK", label="first", height=31, and location-qualified URL; $render data are secret="SECOND-LEAK", label="second", height=62, and location-qualified URL`,
  path.startsWith("action")
    ? "render the authored document, activate its exact rendered control, execute actual $render with replacement data, then activate the exact replacement control"
    : "render the authored document, then execute actual $render with replacement data",
  productionPipelineExpected[path] ?? `on initial render and $render, exactly one component iframe selected by srcdoc data-red-location=${q(path)} retains literal {{$jason.secret}} and has wrapper heights 31px then 62px; exactly one exclusive URL iframe changes from ${q(`https://example.com/${path}-first.html`)} to ${q(`https://example.com/${path}-second.html`)}; exact labels change from ${q(`${path}-first`)} to ${q(`${path}-second`)}${productionPipelineExtra[path] ?? ''}`,
));
...
add(SANDBOX_OBSERVER_CASES, ({ route }) => c(
  `a tracked iframe, resettable event log, and configured sandbox mutation route ${q(route)}`,
  `apply sandbox mutation sequence: ${sandboxMutationDetails[route]}`,
...
add(SOURCE_OBSERVER_CASES, ({ route }) => {
  ...
  return c(
    `a tracked unsandboxed iframe, resettable event log, and configured source mutation route ${q(route)}`,
    `apply source mutation sequence: ${sourceMutationDetails[route]}`,
...
const literalContracts = {
  ...
  "an embedded raw body action interpolates when its separate action phase executes": c('embedded $set options probe {type:"html", text:"{{$jason.value}}"}, body context value "BODY", and action context value "LATER"', "body-transform the action, observe its raw text, then execute that exact transformed action", 'immediate transformed text equals "{{$jason.value}}" and local.probe equals {type:"html", text:"LATER"}'),
  ...
  "integrated component marker completes before public return and later iframe events are rejected": c('an integrated component observer, wrapper, iframe, source value, and a configured late-mutation value', 'inside the containing public boundary set sandbox and source, append the iframe, mark data-jasonette-type="html" completion, then mutate the source late', 'trace equals [CREATE(component), SANDBOX("allow-scripts"), SOURCE("srcdoc"), APPEND, RETURN, SOURCE("srcdoc")] and the boundary rejects "iframe event occurred after integrated component completion"'),
  ...
  "body key interpolation does not read an unrelated $jason getter": c('a body-mode object with resolved key "{{key}}" and observable context getters key="field" and $jason={unused:"UNUSED"}', "transform the object while logging context getter access", 'output equals {field:"plain"} and the getter log equals only ["key"]'),
  ...
  "unresolved generic inline expression keeps existing empty substitution": c('authored string "prefix {{missing.deep.value}} suffix" and empty context', "transform with generic options", 'output strictly equals "prefix  suffix"'),
};

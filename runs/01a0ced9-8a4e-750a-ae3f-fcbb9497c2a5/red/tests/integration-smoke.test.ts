...
function productionPipelineTemplate(path: string) {
  ...
  if (path === "header") return {
    header: {
      title: "header-{{$jason.label}}",
      probe: { type: "html", text: "<p>{{$jason.secret}}</p>" },
    },
    sections: [{ items: locationItems(path) }],
  };
  if (path === "footer") return { sections: [], footer: { tabs: { items: locationItems(path) } } };
  if (path === "section") return { sections: [{ items: locationItems(path) }] };
  if (path === "layout") return {
    sections: [{ items: [{ type: "vertical", components: locationItems(path) }] }],
  };
  if (path === "layer") return {
    sections: [{ items: [{ type: "label", text: "layer-foreground" }] }],
    layers: [{ type: "vertical", components: locationItems(path) }],
  };
  ...
  if (path === "nested-array") return {
    nested: [[{ type: "html", text: "<p>{{$jason.secret}}</p>" }]],
    sections: [{ items: locationItems(path) }],
  };
...
let rendererToDeactivate: ReturnType<typeof createRenderer>["renderer"] | undefined;

function createIsolatedRenderer() {
  const created = createRenderer();
  rendererToDeactivate = created.renderer;
  return created;
}

beforeEach(() => document.body.replaceChildren());
afterEach(() => {
  if (rendererToDeactivate) rendererToDeactivate.getState().document = null;
  rendererToDeactivate = undefined;
});

describe("renderer integration", () => {
  it.each(PRODUCTION_BODY_PIPELINE_CASES)("$title", async ({ path }) => {
    const observer = installSecurityObserver();
    const { renderer, root } = createIsolatedRenderer();
    ...
    const assertComponentBoundary = (phase: "first" | "second", height: number) => {
      if (path === "header") {
        expect(root.querySelector(".jasonette-header-title")?.textContent).toBe(`header-${phase}`);
      }
    ...
    const clickActionBoundary = async (phase: "first" | "second", height: number) => {
      const control = exactLabel(root, `run-${path}-${phase}`);
      if (path === "action-options" || path === "action-payload") {
        const authoredAction = JSON.parse(control.getAttribute("data-action")!);
        const rawShape = path === "action-options"
          ? authoredAction.options.actionOptionsProbe
          : authoredAction.options.actionPayloadProbe.payload;
        expect(rawShape.type).toBe("html");
        expect(rawShape.text).toBe("{{$jason.secret}}");
        expect(rawShape.style.height).toBe(height);
      }
      control.click();
    ...
    try {
      if (path === "header" || path === "nested-array") {
        for (const [phase, height] of [["first", 31], ["second", 62]] as const) {
          const transformed = transform(
            productionPipelineTemplate(path),
            { $jason: data(phase, height) },
            { preserveHtmlText: true },
          );
          const rawShape = path === "header"
            ? transformed.header.probe
            : transformed.nested[0];
          expect(rawShape.text).toBe("<p>{{$jason.secret}}</p>");
        }
      }
      renderDocumentObserved(
        renderer,

...
describe("body template transformation boundary", () => {
  ...
  it.each(NESTED_OFF_MODE_ORDER_CASES)("$title", ({ mode }) => {
    const log: string[] = [];
    const calls = new Map<string, number>();
    const context: any = {};
    for (const [name, value] of [
      ...
      Object.defineProperty(context, name, {
        enumerable: true,
        get() {
          log.push(name as string);
          calls.set(name as string, (calls.get(name as string) ?? 0) + 1);
          return value;
        },
      });
    }
    const result = transform({
      "{{outerKey}}": {
        "{{innerAKey}}": "{{innerAValue}}",
        "{{innerBKey}}": "{{innerBValue}}",
      },
      "{{tailKey}}": "{{tailValue}}",
    }, context, mode === "explicit-false" ? { preserveHtmlText: false } : undefined);
    expect(log).toEqual([
      "outerKey", "innerAKey", "innerAValue", "innerBKey", "innerBValue", "tailKey", "tailValue",
    ]);
    ...
    expect(result).toEqual({ child: { alpha: "A", beta: "B" }, tail: "end" });
  });
  ...
  it.each(ORDINARY_DESCRIPTOR_CASES)("$title", ({ kind }) => {
    const input = kind === "ordinary"
      ? { label: "{{value}}" }
      : { "{{field}}": "{{value}}" };
    const context = kind === "ordinary"
      ? { value: "done" }
      : { field: "resolvedLabel", value: "done" };
    const result = transform(input, context);
    expectSafeOwn(expect, result, kind === "ordinary" ? "label" : "resolvedLabel", "done");
  });
  ...
  it("unresolved generic inline expression keeps existing empty substitution", () => {
    const authored = "prefix {{missing.deep.value}} suffix";
    expect(transform(authored, {})).toBe("prefix  suffix");
  });
});

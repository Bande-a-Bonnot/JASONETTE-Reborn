...
function withInheritedGetters(values: Record<string, unknown>, own: Record<string, unknown>) {
  const prototype: any = {};
  const spies: any[] = [];
  for (const [key, value] of Object.entries(values)) {
    const getter = vi.fn(() => value);
    Object.defineProperty(prototype, key, { enumerable: true, configurable: true, get: getter });
    spies.push(getter);
  }
  const object = Object.assign(Object.create(prototype), own);
  Object.defineProperty(object, "inheritedGetterSpies", { value: spies, configurable: true });
  return object;
}
...
function sourceComponent(kind: string) {
  ...
  switch (kind) {
    ...
    case "non-string-text-url": {
      const textToString = vi.fn(() => "<p>must-not-coerce</p>");
      const textRead = vi.fn(() => ({ toString: textToString }));
      Object.defineProperty(component, "text", {
        enumerable: true,
        configurable: true,
        get: textRead,
      });
      component.url = "https://example.com/non-string-text.html";
      component.coercionSpies = [textToString];
      component.requiredRetrievalSpies = [textRead];
      break;
    }
    case "coercion": {
      ...
      component.coercionSpies = [urlToString];
      break;
    }
    case "inherited-only": return withInheritedGetters(
      { text: "<p>inherited</p>", url: "https://example.com/inherited.html" },
      { type: "html" },
    );
    case "inherited-text-own-url": return withInheritedGetters(
      { text: "<p>inherited</p>" },
      { type: "html", url: "https://example.com/own.html" },
    );
    case "inherited-url-own-text": return withInheritedGetters(
      { url: "https://example.com/inherited.html" },
      { type: "html", text: "<p>own</p>" },
    );
    case "missing": break;
...
describe("public HTML component renderer", () => {
  ...
  it("component wrapper return topology and legacy class size border contract remain exact", () => {
    const borderSetter = vi.spyOn(CSSStyleDeclaration.prototype, "border", "set");
    const observer = installSecurityObserver();
    try {
      ...
      expect(borderSetter).toHaveBeenCalledWith("none");
    } finally { observer.restore(); borderSetter.mockRestore(); }
  });
  ...
  it.each(COMPONENT_SOURCE_CASES)("$title", ({ kind, source, expected }) => {
    ...
    try {
      ...
      for (const spy of component.requiredRetrievalSpies ?? []) expect(spy).toHaveBeenCalledTimes(1);
      for (const spy of component.inheritedGetterSpies ?? []) expect(spy).not.toHaveBeenCalled();
    } finally { observer.restore(); }
  });
  ...
  it.each(CSS_CASES)("$title", ({ kind, css, expected }) => {
    ...
    try {
      ...
      if (kind === "inherited") {
        const cssGetter = vi.fn(() => "p{color:inherited}");
        const prototype = {};
        Object.defineProperty(prototype, "css", {
          enumerable: true, configurable: true, get: cssGetter,
        });
        Object.setPrototypeOf(component, prototype);
        Object.defineProperty(component, "inheritedGetterSpy", { value: cssGetter });
      ...
      if (component.inheritedGetterSpy) expect(component.inheritedGetterSpy).not.toHaveBeenCalled();
  ...
  it.each(COMPONENT_TYPE_CASES)("$title", ({ kind, outcome }) => {
    ...
    try {
      ...
      else if (kind === "inherited-html") component = withInheritedGetters(
        { type: "html" },
        { text: "fallback label" },
      );
      ...
      expect(() => {
        ...
        if (outcome === "unknown") {
          ...
          expect(wrapper.classList.contains("jasonette-label")).toBe(false);
          expect(wrapper.getAttribute("data-jasonette-type")).toBe(kind);
        } else {
          expect(wrapper.tagName).toBe("P");
          expect(wrapper.classList.contains("jasonette-label")).toBe(true);
          expect(wrapper.getAttribute("data-jasonette-type")).toBe("label");
    ...
    try {
      ...
      try {
        ...
        observations = {
          ...
          hasLabelClass: wrapper?.classList.contains("jasonette-label"),
      ...
      expect(observations.tagName).toBe("DIV");
      expect(observations.hasLabelClass).toBe(false);
  ...
  it.each(TRANSFORM_RENDER_INHERITANCE_CASES)("$title", ({ kind, source, expected }) => {
    ...
    try {
      let authored: any;
      if (kind === "all-inherited") authored = withInheritedGetters(
        { text: "<p>bad</p>", url: "https://example.com/bad", css: "bad{}" },
        { type: "html" },
      );
      else if (kind === "inherited-text") authored = withInheritedGetters(
        { text: "<p>bad</p>" },
        { type: "html", url: expected },
      );
      else if (kind === "inherited-url") authored = withInheritedGetters(
        { url: "https://example.com/bad" },
        { type: "html", text: expected },
      );
      else authored = withInheritedGetters(
        { css: "bad{}" },
        { type: "html", text: expected },
      );
      ...
      for (const spy of authored.inheritedGetterSpies) expect(spy).not.toHaveBeenCalled();
    } finally { observer.restore(); }
  });
});

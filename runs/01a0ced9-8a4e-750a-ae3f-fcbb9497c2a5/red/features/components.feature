Feature: HTML component source and iframe policy
  ...
  Scenario: component wrapper return topology and legacy class size border contract remain exact
    Given component {type:"html", text:"<p>x</p>"} with the CSSOM border setter observed
    When render through the directly observed component boundary
    Then detached wrapper class list contains "jasonette-html", data type is "html", iframe parent is wrapper, width is "100%", border setter receives "none", and events are [CREATE(component), SANDBOX("allow-scripts"), SOURCE("srcdoc"), APPEND, RETURN]

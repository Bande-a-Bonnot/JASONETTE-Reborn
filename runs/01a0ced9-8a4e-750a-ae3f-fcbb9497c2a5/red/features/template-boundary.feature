Feature: Body-scoped HTML template transformation
  ...
  Scenario Outline: <title>
    ...
    Examples: concrete ordinary_descriptor_cases vectors
      | title | setup | action | expected |
      | ordinary transform output field has the exact own data descriptor | input owns ordinary field label="{{value}}" with context value "done" | transform through the public template API | output has Object.prototype and own descriptor "label"={value:"done", enumerable:true, writable:true, configurable:true} |
      | resolved transform output field has the exact own data descriptor | input owns resolved key "{{field}}"="{{value}}" with context field="resolvedLabel" and value="done" | transform through the public template API | output has Object.prototype and own descriptor "resolvedLabel"={value:"done", enumerable:true, writable:true, configurable:true} |
  ...
  Scenario Outline: <title>
    ...
    Examples: concrete nested_off_mode_order_cases vectors
      | title | setup | action | expected |
      | nested omitted off mode preserves per-entry key-then-value order exactly once in every frame | resolved outer child and tail entries plus two resolved inner entries with observable key/value getters, options omitted | transform while logging every nested getter access | log equals ["outerKey","innerAKey","innerAValue","innerBKey","innerBValue","tailKey","tailValue"] and every getter count is one |
      | nested explicit false off mode preserves per-entry key-then-value order exactly once in every frame | resolved outer child and tail entries plus two resolved inner entries with observable key/value getters, preserveHtmlText=false | transform while logging every nested getter access | log equals ["outerKey","innerAKey","innerAValue","innerBKey","innerBValue","tailKey","tailValue"] and every getter count is one |
  ...
  Scenario: unresolved generic inline expression keeps existing empty substitution
    Given authored string "prefix {{missing.deep.value}} suffix" and empty context
    When transform with generic options
    Then output strictly equals "prefix  suffix"

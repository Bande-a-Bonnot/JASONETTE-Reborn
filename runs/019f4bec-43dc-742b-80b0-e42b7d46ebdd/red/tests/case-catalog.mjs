...
export const INHERITED_COMPONENT_CALLABLE_CASES = [
  ...
  title: `component registry inherited callable: ${name} is not invoked and exact unknown oracle is returned`,
  name,
}));
...
add(COMPONENT_TYPE_CASES, ({ kind, outcome }) => c(
  ...
  "render the component through the public component API",
  outcome === "unknown" ? `no throw, iframe, or trace; text is ${q(`Unknown component: ${kind}`)} and data-jasonette-type is ${q(kind)}` : `no throw, iframe, or trace; visible text is ${kind === "label" ? '"visible value"' : '"fallback label"'}`,
));
add(INHERITED_COMPONENT_CALLABLE_CASES, ({ name }) => c(
  ...
  "render the component through the public component API",
  `call count is zero, no iframe or trace exists, text is ${q(`Unknown component: ${name}`)}, and data-jasonette-type is ${q(name)}`,
));
...
const backgroundOutcome = (source, expected, extra = "") => `one root-child iframe class="jasonette-background-web" and aria-hidden="true" precedes foreground, has sandbox="allow-scripts", only ${source}="${expected}", and events [CREATE(background), SANDBOX("allow-scripts"), SOURCE("${source}"), APPEND, RETURN]${extra}`;
...
const literalContracts = {
  ...
  "successful continuation interpolates HTML-shaped options generically in exact order": c('primary $set completion plus success $set HTML probe using observable secret="CONTINUED" and height=12 getters', "execute the primary action and its success continuation", 'primaryCompleted is true; getter order is ["secret-after-primary","height-after-primary"]; continuation probe is {type:"html", text:"<p>CONTINUED</p>", style:{height:12}}'),
  ...
  "inherited session domain cannot decorate request headers or body": c('inherited api.example.com session has forbidden header and body; own POST has X-Own-Request="kept-exactly" and data {own:"kept"}', "execute POST https://api.example.com/resource", 'fetch occurs once; own header remains, inherited header is absent, and body code units equal {"own":"kept"} without inherited sentinels'),

Feature: Action dispatch and safe state boundaries
  ...
  Scenario: an embedded raw body action interpolates when its separate action phase executes
    Given embedded $set options probe {type:"html", text:"{{$jason.value}}"} and action context value "LATER"
  ...
  Scenario Outline: <title>
    ...
    Examples: concrete named_action_array_cases vectors
      | title | setup | action | expected |
      | own named action arrays execute async non-commutative authored order through trigger | own arrayAction contains GET https://order.example.com/first then success write "first", followed by GET https://order.example.com/second then success write previous+"->second"; invocation uses {trigger:"arrayAction"} | await the named array dispatch | two fetches occur; log equals ["first-request-start","first-request-resolved","first-write","second-request-start","second-request-resolved","second-write-after-first"] and final value is "first->second" |
      | own named action arrays execute async non-commutative authored order through lambda | own arrayAction contains GET https://order.example.com/first then success write "first", followed by GET https://order.example.com/second then success write previous+"->second"; invocation uses {$lambda options {name:"arrayAction"}} | await the named array dispatch | two fetches occur; log equals ["first-request-start","first-request-resolved","first-write","second-request-start","second-request-resolved","second-write-after-first"] and final value is "first->second" |

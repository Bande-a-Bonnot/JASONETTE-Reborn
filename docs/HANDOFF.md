...
|------------------------|----------------------|---------------------------|----------------|
| `$set`, `$cache.set`, `$cache.reset`, `$flush`, `$render`, `$reload`, `$href`, `$back`, `$close`, `$network.request`, `$lambda`/`trigger`, `$util.alert`, `$util.toast`, `$util.banner`, `$timer.start`, `$timer.stop`, `$audio.play`, `$media.camera`, `$media.picker`, `$media.play`, `$snapshot`, `$util.share`, `$util.addressbook`, `$log` | `$get`, `$cache.get`, `$script.include` | `$util.picker`, `$util.datepicker`, `$vision.scan` | `$geo.get` permission appears but direct fixture did not visibly render success |

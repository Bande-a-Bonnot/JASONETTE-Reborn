...
- iOS: 558 tests, 0 failures (verified 2026-06-07 after native `$vision.scan` implementation; `swift build` succeeded); generic iOS Simulator `xcodebuild` Debug build succeeded 2026-06-07; direct-entry iPhone 17 Pro simulator QA for action screens completed 2026-06-03 with findings in `docs/qa/2026-06-03-ios-action-screen-qa.md`
...
|------------------------|----------------------|---------------------------|----------------|
| `$set`, `$cache.set`, `$cache.reset`, `$flush`, `$render`, `$reload`, `$href`, `$back`, `$close`, `$network.request`, `$lambda`/`trigger`, `$util.alert`, `$util.toast`, `$util.banner`, `$timer.start`, `$timer.stop`, `$audio.play`, `$media.camera`, `$media.picker`, `$media.play`, `$snapshot`, `$util.share`, `$util.addressbook`, `$geo.get`, `$vision.scan`, `$log` | `$get`, `$cache.get`, `$script.include` | `$util.picker`, `$util.datepicker` | none currently verified |

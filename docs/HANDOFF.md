...
P2:
- `todos/049-open-p2-ios-network-eliza-demo-parse-warning.md` — Action → `$network` → Eliza demo shows `The data couldn’t be read because it isn’t in the correct format.` during simulator QA; investigate fixture/server drift vs renderer-side parsing/action mismatch.
